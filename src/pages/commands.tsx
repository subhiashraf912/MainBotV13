...
const CommandsPage = ({commands:clientCommands, user, clientUser}:InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const getcommandsByCategory = (category: string) => commands.filter(cate => cate.category.toLowerCase() === category.toLowerCase())!;
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  const { req, res } = ctx;
  const { cookies } = req;
  const commandsResponse = await fetch(`${config.dashboard}/api/client/commands`);
  ...
  const commands: BaseCommand[] = await commandsResponse.json();
    ...
    const profileResponse = await fetch(`${config.dashboard}/api/user/profile`, {
      headers: {
