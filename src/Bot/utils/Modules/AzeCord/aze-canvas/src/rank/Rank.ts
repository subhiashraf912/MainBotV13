...
export default class RankCard {
	...
	xpCurrent: number;
	xpNeeded: number;
	addonReputation: boolean;
	addonRank: boolean;
	addonRankName: boolean;
	addonBadges: boolean;
	colorBackground: string;
	colorLevelBox: string;
	colorReputationBox: string;
	colorLevel: string;
	colorRank: string;
	colorRankName: string;
	colorUsername: string;
	colorReputation: string;
	colorBackgroundBar: string;
	colorNeededXp: string;
	colorBar: string;
	colorNoBadges: string;
	colorBadgesBox: string;
	radiusCorner: string;
	opacityAvatar: string;
	opacityBadges: string;
	opacityLevel: string;
	opacityBackgroundBar: string;
	opacityReputation: string;
	opacityNoBadges: string;
	textLevel: string;
	textNeededXp: string;
	textReputation: string;
	badge1: string | null;
	badge2: string | null;
	badge3: string | null;
	badge4: string | null;
	...
	setXP(variable: string, value: any) {
	...
	setColor(variable: string, value: any) {
	...
	setText(variable: string, value: any) {
	...
	setOpacity(variable: string, value: any) {
	...
	setAddon(variable: string, value: any) {
	...
	setBadge(variable: any, value: null) {
	...
	async toAttachment() {
		...
		const lvlText = this.textLevel.replaceAll(/{level}/g, this.level);
		const repText = this.textReputation.replaceAll(
			/{reputation}/g,
			this.reputation,
		);
		...
		ctx.globalAlpha = parseInt(this.opacityAvatar);
		...
		ctx.globalAlpha = parseInt(this.opacityLevel);
		...
		if (this.addonReputation) {
			...
			ctx.globalAlpha = parseInt(this.opacityReputation);
		...
		if (this.addonBadges) {
			...
			ctx.globalAlpha = parseInt(this.opacityBadges);
			ctx.fillRect(240 + 50 + 50, 295, 700, 70);
			ctx.fillStyle = this.colorNoBadges;
			if (!this.badge1) {
				ctx.globalAlpha = parseInt(this.opacityNoBadges);
				ctx.textAlign = "center";
		...
		const textXPEdited = this.textNeededXp
			.replaceAll(/{needed}/g, this.xpNeeded.toString())
			.replaceAll(/{current}/g, this.xpCurrent.toString())
			.replaceAll(/{latest}/g, latestXP.toString());
		ctx.textAlign = "center";
		...
		ctx.globalAlpha = parseInt(this.opacityBackgroundBar);
