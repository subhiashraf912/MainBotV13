  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    if (args[0]) {
        ...
        if (relatedCommand) {
          message.reply({
            content: GetLanguage(
              ...
              config.language
            ).replace("{command}", relatedCommand.getName()),
          });
        } else {
          message.reply({
      ...
      const embed = new MessageEmbed()
        .setDescription(
          `**${GetLanguage("BotCommandHelp", config.language).replace(
            "{bot}",
        ...
        .addField(
          ...
          `\`\`\`${
            command.getBotPermissions()[0]
              ? command
                  .getBotPermissions()
                  .map((perm) => `${formatString(perm.replace("_", " "))}`)
        ...
        .setFooter(
          GetLanguage("BotsCommandsHelpSection", config.language).replace(
            "{bot}",
    ...
    const emojis: any = {
      music: "🎶",
      owner: "<:874134305629159496:883433746613796895>",
      owners: "💥",
      developers: "🛠",
      testing: "🧪",
      utils: "🔰",
    ...
    const embed = new MessageEmbed()
      ...
      .addFields(
        directories.map((category) => {
          return {
            name: `> ${emojis[category.toLowerCase()] || ""} ${formatString(
              category
            )}`,
            value: `\`\`\`css\n${GetLanguage(
    ...
    collector.on("collect", (interaction: any) => {
      ...
      const categoryEmbed = new MessageEmbed()
        .setTitle(
          GetLanguage("HelpCategoryEmbedTitle", config.language).replace(
            "{category}",
            formatString(directory)
          )
        )
        .setDescription(
          GetLanguage("HelpCategoryEmbedDescription", config.language).replace(
            "{prefix}",
            prefix
          )
        )
