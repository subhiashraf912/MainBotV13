...
export default class Command extends BaseCommand {
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    if (!args[0] || !args[1]) {
      message.reply({
        content: GetLanguage("AddLevelRolesWrongSyntax", language).replace(
          "{prefix}",
          prefix
        ),
      });
    ...
    if (isNaN(level)) {
      message.reply({
        content: GetLanguage("AddVoiceLevelRolesWrongSyntax", language).replace(
          "{prefix}",
          prefix
        ),
      });
    ...
    message.reply({
      content: GetLanguage("LevelRolesUpdated", language)
        ...
        .replace("{level}", millisToMinutesAndSeconds(level)),
    });
  }
}
